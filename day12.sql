...
-- */
--declare @input varchar(max) = '{"c":"violet","a":8,"b":["red",{"a":37},"green",84,"yellow","green",[24,45,"blue","blue",56,"yellow"],"orange"]}'
...
declare @thisstr varchar(max) = ''
declare @thisbit varchar(max) = ''
declare @lvl int = 0
declare @inkey bit = 0
declare @inobj int = 0
...
declare @totval int = 0

-- part 1, walk the string and find the numbers
while @cnt < len(@input)
begin
	set @cnt = @cnt + 1
	set @thisc = SUBSTRING(@input, @cnt, 1)
	if @thisc in ('{','[') -- open list/object
		set @lvl = @lvl + 1
	else if @thisc = '"'
		if @inkey = 1
		begin
	...
	else if @thisc in ('}',']') -- end list / object
	begin
		if ISNUMERIC(@thisstr) = 1
			set @totval = @totval + @thisstr
		set @thisstr = ''
		set @lvl = @lvl - 1
	end
	else if @thisc = ','
	begin
		...
		-- this is a comma after a list of object, ignore
	end
	else if @thisc not in (':',' ') --ignore whitespace and the key/value sep
		set @thisstr = @thisstr + @thisc
end

raiserror('part 1, total value = %d', 0,0,@totval) with nowait

drop table if exists #data
create table #data(id int identity (1,1), pid int, s varchar(max), inobj bit, spos int, epos int, totval int, red bit)

-- part 2, split the string into a table of data at that level
set @cnt = 0
while @cnt < len(@input)
begin
	set @cnt = @cnt + 1
	...
	if @thisc in ('{','[') -- open list/object
	begin
		if @thisc = '{' set @inobj = 1 else set @inobj = 0
		set @cnt2 = @cnt
		set @lvl = 1
		set @thisstr = ''
		set @totval = 0
		set @spos = @cnt
		while @cnt2 < len(@input)
		begin
			set @cnt2 = @cnt2 + 1
			set @thisc = SUBSTRING(@input, @cnt2, 1)
			if @thisc in ('{','[') -- open list/object
				set @lvl = @lvl + 1
			else if @thisc in ('}',']') -- end list / object
			begin
				...
				begin
					set @epos = @cnt2
					if @thisbit <> ''
						if ISNUMERIC(@thisbit) = 1
							set @totval = @totval + @thisbit
					set @thisbit = ''
					insert into #data (s, inobj, spos, epos, totval)
					values (@thisstr, @inobj, @spos, @epos, @totval)
					raiserror('found subsection  %s, inobj=%d (%d-%d)',0,0, @thisstr, @inobj, @spos, @epos)
					break
				end
			end
			if @lvl = 1
			begin
				...
				begin
					if @thisbit <> ''
					begin
						raiserror('found section  %s',0,0, @thisbit)
						if ISNUMERIC(@thisbit) = 1
							set @totval = @totval + @thisbit
					end
					set @thisbit = ''
				end
				else set @thisbit = @thisbit + @thisc
			end
			else 
				set @thisstr = @thisstr + '_'
		end
	end

end

-- find the parent structure, and identify 'red' lines
update d
set pid = d2.id,
	red = case when d.s like '%:"red"%' and d.inobj = 1 then 1 else 0 end
from #data d
outer apply (
	select top 1 * from #data
	where spos < d.spos and epos > d.epos
	order by (epos - spos) asc
	) as d2

-- update first children of reds, run this a few times till the count stops :)
while 1=1 
begin
	update d
	set red = 1
...
select sum(totval) part1, sum(case when red = 0 then totval else 0 end) part2 from #data
